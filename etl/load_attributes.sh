#!/bin/bash
set -e
export BASEDIR=/home/bitner/devseed/wb-covid/workspace/CoVID/
export OUTPUTDIR=/home/bitner/devseed/wb-covid/workspace/output/
export RELOAD=true
...
function load {
    ...
    echo $add_columns
    echo $add_commas
    sed -e "1 s/$/${add_columns}/" -e "2,$ s/$/${add_columns}/" $file | \
        csvcut -c "$cols" -x | \
        csvcut -K 1 >> ${OUTPUTDIR}/${base}_${grp}_stats.csv
}
export -f load
...
function aggregate_csv {
    ...
    find $BASEDIR | \
        ...
        parallel -j $NJOBS "load {} $base $grp $cols"
}
export -f aggregate_csv
...
# makes sure everything is run for each pattern
function load_all {
    # aggregate_csv $1 $2 base
    aggregate_csv $1 $2 dhs
    load_csv $2 base
    # load_csv $2 dhs
}
...
# recreate tables
for table in adm0 adm1 adm2 urban_areas urban_areas_hd urban_fishnets hd_urban_fishnets
do
base_pgcols=$(tablecols $base_cols)
dhs_pgcols=$(tablecols $dhs_cols)

psql <<EOSQL
    BEGIN;
    DROP TABLE IF EXISTS ${table}_base_stats;
    CREATE TABLE IF NOT EXISTS ${table}_base_stats (
        ogc_fid int,
        ${base_pgcols}
    );

    DROP TABLE IF EXISTS ${table}_dhs_stats;
    CREATE TABLE IF NOT EXISTS ${table}_dhs_stats (
        ogc_fid int,
        ${dhs_pgcols}
    );
    COMMIT;
...
load_all "[^_]urban_[0-9]+" urban_fishnets
